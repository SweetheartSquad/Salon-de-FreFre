	...
	playerCam(_playerCam)
{
	...
	lashes = new MeshEntity(meshes.at(2), _shader);
	eyes = new MeshEntity(meshes.at(3), _shader);
	cheeks = new MeshEntity(meshes.at(4), _shader);
	lips = new MeshEntity(meshes.at(5), _shader);
	...
	lashes->mesh->pushTexture2D(MY_ResourceManager::globalAssets->getTexture("original")->texture);
